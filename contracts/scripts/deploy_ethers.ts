...
async function main() {
  ...
  console.log("\nConfiguring roles...");
  const invABI = load("InvoiceNFT").abi;
  const inv = new ethers.Contract(invoiceNFTAddress, invABI, wallet);
  ...
  const vmABI = load("VaultManager").abi;
  const vm = new ethers.Contract(vaultManagerAddress, vmABI, wallet);
  ...
  // 7. Register agent
  const acABI = load("AgentController").abi;
  const ac = new ethers.Contract(agentControllerAddress, acABI, wallet);
  ...
  console.log(`AGENT_SIGNER_ADDRESS=${wallet.address}`);
}
